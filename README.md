...
Правила разработки и безопасные проверки: [DEVELOPMENT.md](docs/DEVELOPMENT.md).
Инструкции для Codex: [AGENTS.md](AGENTS.md). Результаты аудита и приоритеты
исправлений: [AUDIT-2026-09-07.md](docs/AUDIT-2026-09-07.md).
...
`docker compose down -v` удаляет volumes проекта, включая данные MySQL и Redis.
Применяйте эту процедуру только к нужному локальному стенду при наличии
проверенной резервной копии; для проверки кода она не требуется.
...
| `pip install -r requirements.txt`                                                     | установить runtime-зависимости для запуска |
| `pip install -r requirements-dev.txt`                                                 | дополнительно установить зависимости для разработки и тестов |
| `cp .env-default .env`                                                                | создать локальный `.env` |
...
Миграции сохраняют корректные legacy-дубли ИНН: `organization.inn` имеет
неуникальный индекс. Новый или изменённый ИНН при ручном сохранении проверяется
на занятость отдельно. Downgrade последней миграции возвращает UNIQUE и может
завершиться ошибкой при дублях. Начальную legacy-схему нужно подготовить до
`alembic upgrade head`; цепочка миграций не создаёт все таблицы с нуля.
...
| Команда | Описание |
|------------|-------------------------------------|
| `make requirements` | установить зависимости для разработки |
...
| `make test-unit` | запустить изолированные тесты без рабочей БД и ключей Dadata |
| `make check-docs` | проверить инструкции, навыки и локальные ссылки |

`make test` включает тесты, создающие и удаляющие записи в БД из конфигурации
приложения. Для обычной проверки используйте `make test-unit`; полный набор
запускайте на одноразовой MySQL-схеме по [DEVELOPMENT.md](docs/DEVELOPMENT.md).
